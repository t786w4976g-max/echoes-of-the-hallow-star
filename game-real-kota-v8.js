(() => {
  ...
  beginButton.disabled=true;beginButton.textContent='Loading Kota…';const modelStatus=$('model-status');modelStatus.classList.add('show');modelStatus.textContent='Loading real Kota model…';
  BABYLON.SceneLoader.ImportMeshAsync('', './', 'Kota_REAL_v8.glb?v=8', scene).then(result=>{
    ...
    const topRoots = [...allImported].filter(node => !node.parent || !allImported.has(node.parent));
    const armatureRoot = topRoots.find(node => (node.name || '').toLowerCase().includes('armature')) || topRoots[0];
    if (!armatureRoot) throw new Error('No imported armature root found');
    armatureRoot.parent = visualRoot;
    ...
    // Meshy exported the model with its own 0.01 armature scale, so no extra scaling is needed.
    ...
    kotaReady = true;
    modelStatus.textContent='Real Kota loaded';setTimeout(()=>modelStatus.classList.remove('show'),900);
    playKotaAnimation('idle', true);
    beginButton.disabled = false;
    beginButton.textContent = 'Begin';
    console.log('Kota loaded', {
    ...
    modelStatus.classList.add('show');modelStatus.textContent='ERROR: real Kota GLB did not load. No placeholder character is being used.';
