(() => {
  ...
  function kota(x,z){
    ...
    [torso,waist,belt,head,hair,pend,armL,armR,brL,brR,fistL,fistR,legL,legR,bootL,bootR].forEach(m=>shadows.addShadowCaster(m));
    return {root,fistL,fistR,armL,armR,pend};
  }

  const player=new BABYLON.TransformNode('PlayerRoot',scene);player.position.set(0,h(0,15),15);
  const visualRoot=new BABYLON.TransformNode('KotaVisualRoot',scene);visualRoot.parent=player;
  visualRoot.rotation.y=Math.PI;
  let kotaReady=false,currentAnim='';
  const kotaAnims={};
  const beginButton=$('begin');
  beginButton.disabled=true;beginButton.textContent='Loading Kota…';
  BABYLON.SceneLoader.ImportMeshAsync('', 'assets/', 'Kota_Mobile_Merged.glb', scene).then(result=>{
    const importedSet=new Set(result.meshes);
    result.meshes.forEach(m=>{
      if(!m.parent || !importedSet.has(m.parent)) m.parent=visualRoot;
      if(m.getTotalVertices && m.getTotalVertices()>0){shadows.addShadowCaster(m);m.receiveShadows=true;}
    });
    visualRoot.scaling.setAll(1.0);
    const bounds=result.meshes.filter(m=>m.getBoundingInfo).map(m=>m.getBoundingInfo().boundingBox);
    result.animationGroups.forEach(g=>{kotaAnims[(g.name||'').toLowerCase()]=g;g.stop();});
    kotaReady=true;
    playKotaAnimation('idle',true);
    beginButton.disabled=false;beginButton.textContent='Begin';
  }).catch(err=>{
  ...
  function playKotaAnimation(name,loop=true,speed=1){
    ...
    Object.values(kotaAnims).forEach(g=>g.stop());
    const g=kotaAnims[key];
    if(g){g.start(loop,speed,g.from,g.to,false);currentAnim=key;}
  }
