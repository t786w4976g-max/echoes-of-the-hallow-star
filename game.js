(() => {
  ...
  function kota(x,z){
    const root=new BABYLON.TransformNode('Kota',scene);root.position.set(x,h(x,z),z);
    const torso=BABYLON.MeshBuilder.CreateCapsule('kt',{height:1.5,radius:.38,tessellation:16},scene);torso.parent=root;torso.position.y=1.4;torso.scaling.z=.77;torso.material=M.tunic;
    const waist=BABYLON.MeshBuilder.CreateCylinder('waist',{height:.3,diameterTop:.72,diameterBottom:.78,tessellation:12},scene);waist.parent=root;waist.position.y=.98;waist.material=M.tunic;
    const belt=BABYLON.MeshBuilder.CreateTorus('belt',{diameter:.76,thickness:.065,tessellation:18},scene);belt.parent=root;belt.rotation.x=Math.PI/2;belt.position.y=1.03;belt.material=M.leather;
    const head=BABYLON.MeshBuilder.CreateSphere('kh',{diameter:.69,segments:14},scene);head.parent=root;head.position.y=2.3;head.material=M.skin;
    const hair=BABYLON.MeshBuilder.CreateSphere('khr',{diameter:.77,segments:11,slice:.58},scene);hair.parent=root;hair.position.set(0,2.5,-.02);hair.rotation.x=Math.PI;hair.material=M.hair;
    for(let i=0;i<5;i++){const f=BABYLON.MeshBuilder.CreateSphere('lock'+i,{diameter:.16,segments:6},scene);f.parent=root;f.position.set(-.2+i*.1,2.33-(i%2)*.04,-.3-Math.random()*.04);f.material=M.hair;shadows.addShadowCaster(f)}
    const strap=BABYLON.MeshBuilder.CreateBox('strap',{width:.11,height:1.75,depth:.12},scene);strap.parent=root;strap.position.set(.19,1.55,-.03);strap.rotation.z=.53;strap.material=M.leather;
    const sat=BABYLON.MeshBuilder.CreateBox('sat',{width:.45,height:.48,depth:.24},scene);sat.parent=root;sat.position.set(-.47,1.08,.22);sat.rotation.z=.2;sat.material=M.leather;
    const pend=BABYLON.MeshBuilder.CreatePolyhedron('pend',{type:2,size:.1},scene);pend.parent=root;pend.position.set(0,1.79,-.34);pend.material=M.blue;
    const armL=BABYLON.MeshBuilder.CreateCapsule('al',{height:.95,radius:.105,tessellation:10},scene);armL.parent=root;armL.position.set(-.5,1.43,0);armL.rotation.z=.08;armL.material=M.skin;
    const armR=armL.clone('ar');armR.parent=root;armR.position.x=.5;armR.rotation.z=-.08;
    const brL=BABYLON.MeshBuilder.CreateCylinder('bl',{height:.28,diameterTop:.21,diameterBottom:.23,tessellation:10},scene);brL.parent=root;brL.position.set(-.52,1.1,0);brL.material=M.leather;
    const brR=brL.clone('br');brR.parent=root;brR.position.x=.52;
    const fistL=BABYLON.MeshBuilder.CreateSphere('fl',{diameter:.18,segments:7},scene);fistL.parent=root;fistL.position.set(-.54,.91,0);fistL.material=M.skin;
    const fistR=fistL.clone('fr');fistR.parent=root;fistR.position.x=.54;
    const legL=BABYLON.MeshBuilder.CreateCapsule('ll',{height:1.0,radius:.13,tessellation:10},scene);legL.parent=root;legL.position.set(-.15,.48,0);legL.material=M.pants;
    const legR=legL.clone('lr');legR.parent=root;legR.position.x=.15;
    const bootL=BABYLON.MeshBuilder.CreateBox('bootL',{width:.28,height:.25,depth:.56},scene);bootL.parent=root;bootL.position.set(-.15,.08,.08);bootL.material=M.leather;
    const bootR=bootL.clone('bootR');bootR.parent=root;bootR.position.x=.15;
    [torso,waist,belt,head,hair,strap,sat,pend,armL,armR,brL,brR,fistL,fistR,legL,legR,bootL,bootR].forEach(m=>shadows.addShadowCaster(m));
    return {root,fistL,fistR,armL,armR,pend};
  }

  const K=kota(0,15),player=K.root,rowan=humanoid('Rowan',M.npc,0,-10.5);rowan.rotation.y=Math.PI;
  ...
  function punch(){if(punching||dialogueOpen)return;punching=true;flash('attack');const st=performance.now();const rx=K.fistR.position.x,ry=K.fistR.position.y;const hnd=setInterval(()=>{const p=Math.min(1,(performance.now()-st)/230),a=Math.sin(p*Math.PI);K.armR.rotation.x=-a*.95;K.fistR.position.z=-a*.55;K.fistR.position.y=ry+a*.12;if(p>=1){clearInterval(hnd);K.armR.rotation.x=0;K.fistR.position.set(rx,ry,0);punching=false}},16)}
  ...
  scene.onBeforeRenderObservable.add(()=>{
    ...
    star.rotation.y+=dt*.65;K.pend.scaling.setAll(1+Math.sin(t*3)*.05);sway.forEach(s=>s.g.rotation.y=s.base+Math.sin(t*1.7+s.p)*s.a);
